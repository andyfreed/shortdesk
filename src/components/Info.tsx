...
/**
 * Free-text version of {@link Info} — same "?" affordance, but the caller
 * passes the explanation directly (for settings not in EXPLAINERS).
 */
export function Tip({ title, text }: { title?: string; text: string }) {
  ...
  return (
    <span className="relative inline-flex">
      <button
        type="button"
        aria-label={title ? `What is ${title}?` : "Explanation"}
      ...
      {open && (
        <span
          ...
          onMouseEnter={show}
          onMouseLeave={hideSoon}
          className="absolute left-1/2 top-6 z-50 w-60 -translate-x-1/2 rounded-lg border border-border bg-surface-2 p-3 text-xs leading-relaxed text-foreground shadow-xl"
        >
